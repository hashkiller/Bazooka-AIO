...
namespace TPS
{
	internal class Security
	{
		...
		{
			MD5 mD = MD5.Create();
			Array bytes = Encoding.UTF8.GetBytes(value);
			Array value2 = mD.ComputeHash((byte[])bytes);
			return BitConverter.ToString((byte[])value2).Replace("-", "");
		}
		...
		public static void Start()
		{
			...
			StreamReader streamReader = new StreamReader(pathRoot + "Windows\\System32\\drivers\\etc\\hosts");
		...
		public static string Integrity(string filename)
		{
			...
			Array value = mD.ComputeHash(inputStream);
			return BitConverter.ToString((byte[])value).Replace("-", "").ToLowerInvariant();
		}
